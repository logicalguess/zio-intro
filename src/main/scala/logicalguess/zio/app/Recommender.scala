...
import zio.{DefaultRuntime, Ref, ZIO, console}
...
object Recommender {
  ...
  def tryGettingRecommendationsByEmail(email: String): ZIO[Writer[String], Error, Recommendations] =
    for {
      _         <- log(s"LOG -> Searching for user $email")
      user      <- tryGettingUserByEmail(email)
